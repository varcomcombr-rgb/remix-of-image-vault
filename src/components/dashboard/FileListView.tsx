...
const FileListView = ({ files, onCopy, onDelete, onPreview, selectedFiles, onToggleSelect }: FileListViewProps) => {
  return (
    <>
      {/* Desktop table */}
      ...
      <div className="sm:hidden space-y-2">
        {files.map((file) => (
          <div
            key={file.name}
            className={`flex items-center gap-3 p-3 rounded-xl border bg-card cursor-pointer transition-colors ${
          ...
          >
            ...
            <p className="text-xs font-mono text-foreground truncate flex-1">{file.name}</p>
            <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
              <Button variant="secondary" size="icon" className="h-8 w-8" onClick={() => onCopy(file.url)}>
                <Copy className="w-3 h-3" />
              </Button>
              <Button variant="destructive" size="icon" className="h-8 w-8" onClick={() => onDelete(file.name)}>
                <Trash2 className="w-3 h-3" />
              </Button>
            </div>
          </div>
